# CEE – Computer Engineered Eyes

> **"CEE the chair, CEE the world"** – The Metal Chairs

## Overview

CEE is a project designed to reduce the dangers faced by people with blindness while navigating their environment. Using computer vision, the system detects obstacles and provides real-time auditory guidance to help users move safely and confidently.

## Features

### 🔍 Obstacle Detection
- Identifies objects in front of the user.
- Measures and reports the distance to each object.
- Alerts the user when an object is close enough to pose a potential hazard.
- Provides clear explanations of **what** the object is and **how far** away it is.

### 🪜 Stair Detection & Guidance
- Detects stairs ahead of the user.
- Alerts the user when stairs are present.
- Gives step-by-step guidance for moving **up or down** stairs.
- Reports:
  - How deep each step is.
  - How far the staircase is from the user.

## Purpose

CEE aims to enhance mobility, safety, and independence for individuals with visual impairments through accessible, intelligent, and real-time computer vision assistance.
